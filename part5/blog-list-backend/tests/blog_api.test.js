...
beforeEach(async () => {
    ...
    userObject1.passwordHash = await bcrypt.hash(initialUsers[0].password, saltRounds)
    const user = await userObject1.save()


    let userObject2 = new User(initialUsers[1])
    userObject2.passwordHash = await bcrypt.hash(initialUsers[1].password, saltRounds)
...
describe('user with access', () => {
    ...
    test('delete blog', async () => {
        const deleteResponse = await api.delete('/api/blogs/5a422bc61b54a676234d17fc')
            .set('Authorization', `Bearer ${token}`)
